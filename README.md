...
exposed port with `docker ps`:
        ...
        docker run --env-file .env --P capstone-auth
        docker ps
...
shell):
        ...
        docker run --env-file .env capstone-auth npm run shell

To run a Django shell inside the container:

        docker run --env-file .env -t capstone-auth
